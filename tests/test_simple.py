...
@patch("your_module.Orbit")
@patch("your_module.get_times")
@patch("your_module.rv")
def test_ssapy_orbit_with_keplerian(mock_rv, mock_get_times, mock_orbit):
    ...
    r, v = ssapy_orbit(a=7000e3)
    assert r.shape == v.shape == (3, 3)
    assert len(t) == 3


@patch("your_module.points_on_circle")
@patch("your_module.ssapy_orbit")
...
@patch("your_module.rv", side_effect=RuntimeError("Failed"))
@patch("your_module.get_times")
@patch("your_module.Orbit")
